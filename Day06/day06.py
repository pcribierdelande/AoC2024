...
def compute_next_possible_position(
...
def get_next_guard_position(
    current_position: tuple[int, int], direction: str, obstacles: list[tuple[int, int]]
...
def arpenter(
    ...
    while not check_guard_outside(current_position, x_lim, y_lim):
        current_position, direction = get_next_guard_position(
            current_position, direction, obstacles
        )
