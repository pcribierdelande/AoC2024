...
def read_input(input_file: str) -> tuple[np.ndarray[str, str], list[str]]:
    ...
    all_antena_type = [a[0] for a in list(np.unique(grid)) if a[0] != '.']
    return grid, all_antena_type


def get_antinodes_from_two_antenas(antena1: tuple[int, int], antena2: tuple[int, int]) -> tuple[list[tuple[int, int]], list[tuple[int, int]]]:
    x1, y1 = antena1
    x2, y2 = antena2
    diff_x = x1-x2
    diff_y = y1-y2
...
def check_antinode_in_grid(antinode_coord: tuple[int, int], x_lim: int, y_lim: int) -> bool:
    x, y = antinode_coord
    return not (x < 0 or y < 0 or x >= x_lim or y >= y_lim)


def level1(grid: np.ndarray[str, str], all_antena_type: list[str], x_lim: int, y_lim: int) -> int:
    antinodes = []
    for antena_type in all_antena_type:
        ...
        for antena_couple in list(itertools.combinations(antena_coords, 2)):
            possible1, possible2 = get_antinodes_from_two_antenas(antena1=antena_couple[0], antena2=antena_couple[1])
            if len(possible1) > 0:
...
def level2(grid: np.ndarray[str, str], all_antena_type: list[str], x_lim: int, y_lim: int) -> int:
    antinodes = []
    for antena_type in all_antena_type:
        xs,ys = np.where(grid == antena_type)
        antena_coords = [(int(x), int(y)) for x,y in zip(xs,ys)]
